...
-- finding duplicates using the same fields as in the surragte key then keeping only one of the records
with tripdata as 
(
  select *,
    row_number() over(partition by vendor_id, pickup_datetime) as rn
  from {{ source('staging','green_tripdata') }}
  -- making sure we don't bring in any null data by checking vendor id is not empty
...
select
    ...
    {{ dbt.safe_cast("dropoff_location_id", api.Column.translate_type("integer")) }} as pickup_locationid,
